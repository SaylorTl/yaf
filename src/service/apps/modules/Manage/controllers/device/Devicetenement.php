...
class Devicetenement extends Base {
    ...
    {
        if (!isTrueKey($params, ...['page', 'pagesize', 'device_id'])) rsp_error_tips(10001, 'page pagesize device_id');
        ...
        if (!$house_ids) rsp_success_json(['total' => 0, 'lists' => []]);
        $house_ids = array_unique(array_filter(array_column($house_ids, 'house_id')));

        // tenement_ids
        $tenement_ids = $post = [];
        if (isset($params['real_name']) && trim($params['real_name'])) $post['real_name'] = $params['real_name'];
        if (isset($params['mobile']) && trim($params['mobile'])) $post['mobile'] = $params['mobile'];
        if ($post) {
            $tenement_ids = $this->user->post('/tenement/userlist',array_merge($post, ['page' => 1, 'pagesize' => 99999]));
        ...
        if (isTrueKey($params, 'tenement_identify_tag_id')) $post['tenement_identify_tag_id'] = $params['tenement_identify_tag_id'];
        if ($tenement_ids) $post['all_tenement_ids'] = $tenement_ids;
        ...
        $tenements = $this->user->post('/tenement/userlist',['tenement_ids' => $tenement_ids, 'house_ids' => $house_ids, 'page' => 1, 'pagesize' => 99999]);
