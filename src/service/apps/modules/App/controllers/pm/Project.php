...
final class Project extends Base
{
    public function lists($params = [])
    {
        ...
        if ($user_id) {
            $tenements = $this->user->post('/tenement/lists', ['user_ids' => [$user_id], 'tenement_check_status' => 'Y']);
            ...
            $project_ids = array_unique(array_filter(array_column($houses,'project_id')));
            if (isTrueKey($params, 'project_id')) $project_ids = array_intersect($project_ids, [$params['project_id']]);
            if (!$project_ids) rsp_success_json(['total' => 0,'lists' => []]);
    ...
    const prefix = 'client_wechat_project';
    const redis_db = 8;
    /**
     * @param array $params
     * 获取用户上次选择项目
     */
    public function get_project($params = []){
        if(isTrueKey($params,'client_id') == false) rsp_error_tips(10001,'client_id');
        $key = self::prefix;
        $redis = Comm_Redis::getInstance();
        $redis->select(self::redis_db);
        $project = $redis->hget($key,$params['client_id']);
        if(!$project) rsp_error_tips(10013);
        rsp_success_json(json_decode($project,true));
    }
     ...
     * 设置用户所选项目
     */
    public function set_project($params = []){
        if(isTrueKey($params,'client_id') == false) rsp_error_tips(10001,'client_id');
        if(isTrueKey($params,'project_id') == false) rsp_error_tips(10001,'project_id');
        if(isTrueKey($params,'project_name') == false) rsp_error_tips(10001,'project_name');
        $key = self::prefix;
        $redis = Comm_Redis::getInstance();
        $redis->select(self::redis_db);
        $data = [
        ...
        $redis->hset($key,$params['client_id'],json_encode($data,JSON_UNESCAPED_UNICODE));
        rsp_success_json(1);
    }
}