...
class Basecharging extends Base
{
    ...
    private function property_management_fee($params)
    {
        ...
        if ($space_show['code'] != 0 || !$space_show['content']) {
            rsp_die_json(10002, '空间信息不存在');
        }
    ...
    private function station_fee($params)
    {
        ...
        if (!isTrueKey($params, 'project_id') && empty($params['mobile'])) {
            return $return;
        }
